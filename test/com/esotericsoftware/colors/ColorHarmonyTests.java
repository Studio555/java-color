...
public class ColorHarmonyTests {
	...
	public void testTriadic () {
		...
		HSL base = HSL(red);
		HSL color2 = HSL(triadicRed[1]);
		HSL color3 = HSL(triadicRed[2]);
		...
		assertClose(0, HSL(rgbTriad[0]).H(), "Red hue", 1);
		assertClose(120, HSL(rgbTriad[1]).H(), "Green hue", 1);
		assertClose(240, HSL(rgbTriad[2]).H(), "Blue hue", 1);

		// Test with green
		RGB green = new RGB(0, 1, 0);
		RGB[] triadicGreen = triadic(green);
		// Green (120°) -> Blue (240°) -> Red (360°/0°)
		assertClose(120, HSL(triadicGreen[0]).H(), "Green hue", 1);
		assertClose(240, HSL(triadicGreen[1]).H(), "Blue hue", 1);
		float redHue = HSL(triadicGreen[2]).H();
		...
		HSL testBase = HSL(testTriadic[0]);
		HSL test2 = HSL(testTriadic[1]);
		HSL test3 = HSL(testTriadic[2]);
		...
		for (float hue = 0; hue < 360; hue += 45) {
			...
			float h1 = HSL(triad[0]).H();
			float h2 = HSL(triad[1]).H();
			float h3 = HSL(triad[2]).H();
