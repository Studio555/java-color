...
public class CCTTests extends Tests {
	...
	public void testxyErrorOhno () {
		float max = 0;
		for (float K = 1667; K < 25000; K += 0.1f) {
		...
		assertTrue(max < 3.14f, "Max error: " + max);
	}
	...
	@Test
	public void testRobertsonInflectionPoint () {
		// The inflection point where slopes change sign is around 1600-1700K
	...
	@Test
	public void testRobertsonHandlesInflectionPoint () {
		// This test verifies that the implementation handles the
		// inflection point region correctly. The user changed slopes
		// to negative after 600 mired to avoid interpolation issues.
		...
		// Temperature should be reasonable (user mentioned ~1620K)
		assertTrue(result.K() > 1500 && result.K() < 1700, "Temperature should be in inflection point range, was " + result.K());

		// The key insight: With positive slopes after 600 mired and
		// no special handling, linear interpolation between -116 and +627
		// would give a small slope near 0, causing incorrect results.
		// The negative slopes avoid this by keeping all slopes negative.

		// This demonstrates the pragmatic solution works, even if not
		// theoretically perfect per Robertson 2020.
	}
}
