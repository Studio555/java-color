...
// @Test
// public void testxyError () {
// float max = 0;
// for (float K = 1667; K < 25000; K += 0.1f) {
// float roundTripK = new CCT(K).xy().CCT().K();
// if (Float.isNaN(roundTripK)) System.out.println();
// max = Math.max(max, K - roundTripK);
// }
// assertTrue(max < 4.9, "Max error: " + max); // CCT#xy and xy#CCT interact, giving better accuracy than arbitrary values.
// }

	@Test
	public void testXYZError () {
		// roundTrip(1000, 100000, 0.01f, 3.5585938f); // Slow.
		// roundTrip(1000, 100000, 0.1f, 3.53125f); // Slow.
		roundTrip(2000, 6500, 1, 0.91748047f);
		roundTrip(1000, 7000, 1, 0.9980469f);
		roundTrip(1000, 20000, 1, 2.6855469f);
		roundTrip(1000, 100000, 1, 3.5195312f);
	}

	private void roundTrip (float start, float end, float step, float expectedMaxError) {
		startTimer();
		float max = 0;
		for (float K = start; K < end; K += step) {
			float roundTripK = new CCT(K).XYZ().uv().CCT().K();
			float error = Math.abs(K - roundTripK);
			if (error > max) max = error;
		}
		stopTimer("Time: ");
		assertEquals(expectedMaxError, max, "Wrong max error");
	}
//
// @Test
