...
A number of methods are provided for CCT calculations:

```java
// Improved Robertson method (high accuracy)
...
The [lookup table](https://github.com/EsotericSoftware/color/blob/main/src/com/esotericsoftware/color/space/CCT.java#L349) for that accuracy is generated by [CCTRobertsonLUT](https://github.com/EsotericSoftware/color/blob/main/test/com/esotericsoftware/color/tools/CCTRobertsonLUT.java) and contains only 131 entries: 5 floats each, 655 total. Instead of slope it stores direction, removing a couple `sqrt` calls. Binary and Fibonacci search were benchmarked but slower.

The Ohno table has 515 entries, 1545 floats using 2.3x the memory for worse accuracy.
