...
public class HSLuvTests extends Tests {
	...
	public void testHSLuvReferenceValues () {
		// Test pure colors
		testHSLuvConversion(new RGB(1, 0, 0), 12.177f, 100f, 53.237f, "Pure red");
		testHSLuvConversion(new RGB(0, 1, 0), 127.715f, 100f, 87.737f, "Pure green");
		testHSLuvConversion(new RGB(0, 0, 1), 265.874f, 100f, 32.301f, "Pure blue");
		testHSLuvConversion(new RGB(1, 1, 0), 85.874f, 100f, 97.139f, "Pure yellow");
		testHSLuvConversion(new RGB(0, 1, 1), 192.177f, 100f, 91.114f, "Pure cyan");
		testHSLuvConversion(new RGB(1, 0, 1), 307.715f, 100f, 60.324f, "Pure magenta");

		// Test achromatic colors
		testHSLuvConversion(new RGB(1, 1, 1), 0f, 0f, 100f, "White");
		testHSLuvConversion(new RGB(0, 0, 0), 0f, 0f, 0f, "Black");
		testHSLuvConversion(new RGB(0.5f, 0.5f, 0.5f), 0f, 0f, 53.389f, "50% gray");
	...
	public void testHSLuvEdgeCases () {
		...
		assertEquals(0f, white.S(), 0.1f, "White saturation");
		assertEquals(100f, white.L(), 0.1f, "White lightness");

		// Test black (H should be defined but S should be 0)
		HSLuv black = new RGB(0, 0, 0).HSLuv();
		assertEquals(0f, black.S(), 0.1f, "Black saturation");
		assertEquals(0f, black.L(), 0.1f, "Black lightness");
		...
		for (RGB color : pureColors) {
			HSLuv hsluv = color.HSLuv();
			assertEquals(100f, hsluv.S(), 0.1f, String.format("Saturation for pure color %s", color));
		}
	}
	...
	public void testHSLuvFromValues () {
		...
		HSLuv hsluv = new HSLuv(180f, 50f, 50f);
		RGB rgb = hsluv.RGB();
		...
		for (float r = 0; r <= 1; r += 0.25f) {
			for (float g = 0; g <= 1; g += 0.25f) {
				for (float b = 0; b <= 1; b += 0.25f) {
					...
					assertTrue(hsluv.S() >= 0f, String.format("S >= 0 for %s", rgb));
					assertTrue(hsluv.S() <= 100f, String.format("S <= 100 for %s", rgb));
					assertTrue(hsluv.L() >= 0f, String.format("L >= 0 for %s", rgb));
					assertTrue(hsluv.L() <= 100f, String.format("L <= 100 for %s", rgb));

					if (!Float.isNaN(hsluv.H())) {
						assertTrue(hsluv.H() >= 0f, String.format("H >= 0 for %s", rgb));
						assertTrue(hsluv.H() < 360f, String.format("H < 360 for %s", rgb));
					}
				}
			}
		}
