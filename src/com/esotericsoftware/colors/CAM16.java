...
public record CAM16 (
	...
	public CAM16 lerp (CAM16 other, float t) {
		return new CAM16(Colors.lerp(J, other.J, t), Colors.lerp(C, other.C, t), lerpAngle(h, other.h, t), 0, 0, 0);
	}
	...
	public record VC (float Aw, float Nbb, float Ncb, float c, float Nc, float n, float[] rgbD, float FL, float FLRoot, float z) {
		...
		static public VC with (XYZ wp, float La, float bgL, float surround, boolean discounting) {
			...
			float c = f >= 0.9f ? Colors.lerp(0.59f, 0.69f, (f - 0.9f) * 10) : Colors.lerp(0.525f, 0.59f, (f - 0.8f) * 10);
